...
再看缺点:
q1. 速度慢，Python 的运行速度相比C语言确实慢很多，跟JAVA相 也要慢 些，因此 这也是很多所谓的   屑于使 Python的主要原因，但其实这 所指的运 速度 慢在 多数情况下 户是 法直接感知到的，必须借助测试 具才能体现出来， 如 你 C运 个程序花 0.01s, Python是0.1s,这样C语 直接 Python快 10倍, 算是 常夸张 ，但是你是 法直接通过 眼感知的，因为 个正常 所能感知的时 间最 单位是0.15-0.4s左右，哈哈。其实在 多数情况下Python已经完全可以满  你对程序速度的要求，除 你要写对速度要求极 的搜索引擎等，这种情况下，当 然还是建议你 C去实现的。
2. 代码 能加密，因为PYTHON是解释性语 ，它的源码都是以名 形式存放的，  过我 认为这算是 个缺点，如果你的项 要求源代码必须是加密的，那你 开始就  应该 Python来去实现。
q1q3. 线程 能  多CPU问题，这是Python被 诟病最多的 个缺点，GIL即全局解释  锁(Global Interpreter Lock)，是计算机程序设计语 解释  于同步线程的  具，使得任何时刻仅有 个线程在执 ，Python的线程是操作系统的原 线程。 在Linux上为pthread，在Windows上为Win thread，完全由操作系统调度线程的 执 。 个python解释 进程内有 条主线程，以及多条 户程序的执 线程。即 使在多核CPU平台上，由于GIL的存在，所以禁 多线程的并 执 。关于这个问题 的折衷解决 法，我们在以后线程和进程章节 再进 详细探讨。
