...
str: 字符串 多行赋值一个字符串，用三引号 字符串是不可变的对象，所有任何操作都不会对原字符串有影响
                ...
                9. capitalize() 首字母大写 swapcase() 大小写互相转换 caseflod()转换成小写，和lower（）的区别是。lower（）对某些字符的支持不够好，东欧的一些字符 title（）每个被特殊字符隔开的字符串首字母大写
...
# dic = dict.fromkeys(["XX", "XC"], "YY")
# # 前面列表中的每一项都会作为key，后面列表的内容作为values，生成dict
# print(dic)
# s = "title"
# s1 = s.capitalize()
# print(s1)
